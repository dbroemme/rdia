...
xml.rss :version => "2.0" do
  xml.channel do
    ...
    for post in current_site.posts.where("term_taxonomy.slug in (?)", ["page", "post", "documentation"]).paginate(:page => params[:page], :per_page => current_site.front_per_page).decorate
      xml.item do
